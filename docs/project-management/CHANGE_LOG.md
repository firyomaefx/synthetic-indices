...
|---|---|---|---|
| 2026-09-02 | Offline research package added (`src/break100/research/`): tick-level backtest, line-for-line Python port of the MQL5 box detector, bar-level fill replay, chronological holdout with trial-count-deflated significance, per-box policy (replaces the collapsed HF logistic model), tradeability structure battery, walk-forward metrics. 20-configuration backtest over 41.2 days / 3.56M ticks found no statistically significant edge (best t=+0.12 vs ~2.45 needed); measured 46.7% win rate matches the random-walk prediction from spread geometry. Quarantined a corrupted v1 dataset (MAE mirrored MFE) in favor of a v2 capture schema. | Python research + tools | Owner request |
| 2026-09-02 | v2.23 Shadow ledger runs in every mode for every armed box (was buried 13 returns deep inside ArmBoxOco and gated to SHADOW, so it had never run). Each row tagged with the execution decision, so one ledger gives both filtered and unfiltered performance. Shadow equity persists across restarts. Capture CSVs versioned to v2 after the v1 setup file accumulated 34/39/44-field rows under a 34-field header. | MQL5 Shadow + Capture | Owner request |
| 2026-09-02 | v2.22 Box-size trade filter (InpMinBoxSpreads, default 12 spreads): M30 expectancy -0.164R -> -0.040R, MaxDD 10.1% -> 4.3% over 42 days of ticks. Stop re-anchored to the actual fill when an entry gaps (real fills gapped 60-123 pts vs a 5.00 spread, so the 0.25% risk cap was not real). Broker trade_stops_level (10.0) honoured. Alerts remain ungated. | MQL5 Box + DemoExec | Owner request |
| 2026-09-02 | v2.21 LIVE toggle behind four gates: InpAllowLiveTrading + real account + login allowlist + armed chart button. Arming is never persisted; any fault or halt disarms and cancels pendings. | MQL5 Mode + EA | Owner request |
